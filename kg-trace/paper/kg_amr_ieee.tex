...
Evaluated on the CRYPTIC M. tuberculosis cohort, KG-TRACE achieves an AUROC of 0.9757 for isoniazid, matching fully-tuned baselines despite being trained on 22\% less data. More importantly, we introduce the Biological Grounding Ratio (BGR), a dataset-level metric that quantifies alignment between neural attributions and established biology. Our framework achieves a 94.8\% symbolic coverage of resistant predictions and effectively identifies MDR co-occurrence artifacts by issuing laboratory follow-up flags for 'UNCERTAIN' cases. We demonstrate that neuro-symbolic grounding provides a verifiable audit trail for clinicians, bridging the gap between predictive accuracy and clinical trust.
\end{abstract}
...
{\footnotesize Gate $\alpha = 0.34$ (Genomic-dominant;\\susceptible baseline: 0.22)}
\end{tcolorbox}
\end{minipage}
...
(SHAP top-1 verified by KG trace. \textbf{Dataset BGR:} 0.88)}
\end{tcolorbox}
...
\textcolor{red}{Section~\ref{sec:related} reviews related work. Section~\ref{sec:methods} covers data, KG construction, model architecture, and grounding methodology. Section~\ref{sec:results} presents and interprets results. Sections~\ref{sec:limitations} and~\ref{sec:conclusion} address limitations and conclusions.}
...
\begin{tikzpicture}[
  ...
  darr/.style={->, thick, dashed, red!70},
  elbl/.style={font=\scriptsize, align=center, fill=white, inner sep=1.5pt},
  bblue/.style={rectangle, rounded corners=3pt, draw=black,
    fill=cyan!65!blue, text=white, align=center,
    font=\small\bfseries, inner sep=5pt,
    minimum width=2.6cm, minimum height=0.8cm},
  bpurple/.style={rectangle, rounded corners=3pt, draw=black,
    fill=violet!75, text=white, align=center,
    font=\small\bfseries, inner sep=5pt,
    minimum width=2.6cm, minimum height=0.8cm},
  bred/.style={rectangle, rounded corners=3pt, draw=black,
    fill=red!65, text=white, align=center,
    font=\small\bfseries, inner sep=5pt,
    minimum width=2.6cm, minimum height=0.8cm},
  bgreen/.style={rectangle, rounded corners=3pt, draw=black,
    fill=green!55!black, text=white, align=center,
    font=\small\bfseries, inner sep=5pt,
    minimum width=2.6cm, minimum height=0.8cm},
  byellow/.style={rectangle, rounded corners=3pt, draw=black,
    fill=yellow!45, text=black, align=center,
    font=\small, inner sep=5pt,
    minimum width=3.6cm},
]

%% Entity nodes
\node[bblue]   (gene) at (2.0, 0)    {Gene\\{\scriptsize n = 26}};
\node[bpurple] (mech) at (-0.8, -2.8) {Mechanism\\{\scriptsize n = 9}};
\node[bred]    (mut)  at (4.8, -2.8)  {Mutation\\{\scriptsize n = 25{,}045}};
\node[bgreen]  (drug) at (2.0, -6.0)  {Drug\\{\scriptsize n = 15}};

%% Edges
\draw[arr] (gene.south) to[out=210,in=80]
  node[elbl, left, xshift=-3pt] {\texttt{belongs\_to\_mechanism}\\{\scriptsize (26)}}
  (mech.north);

\draw[arr] (gene.south) to[out=330,in=100]
  node[elbl, right, xshift=3pt] {\texttt{has\_mutation}\\{\scriptsize (25,045)}}
  (mut.north);

\draw[arr] (mut.south) to[out=240,in=10]
  node[elbl, above right, xshift=-10pt, yshift=4pt]
    {\texttt{confers\_resistance\_to}\\{\scriptsize (72)}}
  (drug.east);

\draw[arr] (mut.south) to[out=200,in=350]
  node[elbl, below right, xshift=-8pt, yshift=-4pt]
    {\texttt{confers\_susceptibility\_to}\\{\scriptsize (18,268)}}
  (drug.north east);

\draw[arr] (mech.south) to[out=280,in=160]
  node[elbl, left, xshift=-2pt]
    {\texttt{has\_uncertain\_effect\_on}\\{\scriptsize (11,686)}}
  (drug.west);

\draw[arr] (gene.south) to[out=270,in=90]
  node[elbl, right, xshift=14pt]
    {\texttt{targets}\\{\scriptsize (15)}}
  (drug.north);

%% Highlighted resistance path
\node[byellow, text width=4.0cm, align=left] (pex) at (2.0, -8.5)
  {\textit{WHO Grade-1 Path Example:}\\[2pt]
   $\underbrace{\textit{katG}}_{\text{Gene}}
    \xrightarrow{\texttt{has\_mutation}}
    \underbrace{\textit{katG}\text{:S315T}}_{\text{Mutation}}
    \xrightarrow{\texttt{confers\_resistance\_to}}
    \underbrace{\text{INH}}_{\text{Drug}}$};

\draw[darr] (mut.south) to[out=270,in=80] (pex.north);

%% Stats label
\node[font=\scriptsize, align=left, text=gray!70!black] at (2.0,-9.7)
  {Total: 60,017 triples $\mid$ 25,095 entities $\mid$ 6 relation types};

%% Panel border + label
\draw[rounded corners=6pt, black, very thick]
  (-3.4, 0.8) rectangle (7.6, -10.1);
\node[font=\small\bfseries, anchor=north west] at (-3.4, 0.8)
  {\textit{M.~tuberculosis} AMR Knowledge Graph Schema (Symbolic Component)};

\end{tikzpicture}}
  \caption{\textbf{MTB Knowledge Graph Schema.} The symbolic component encodes 60,017 triples and 25,095 entities from the WHO catalogue. The highlighted path shows a Grade-1 evidence chain used to verify neural SHAP attributions.}
  \label{fig:kg}
\end{figure}
...
\begin{equation}
  \mathbf{h} = \boldsymbol{\alpha} \odot \tilde{\mathbf{g}}
    ...
    \in \mathbb{R}^{128}.
\end{equation}
$\boldsymbol{\alpha}$ can be read as the model's per-sample allocation of trust between the neural genomic evidence and the symbolic KG evidence. Resistant isolates show a higher mean gate value ($\bar{\alpha}=0.34$) than susceptible ones ($\bar{\alpha}=0.22$), which makes sense: confident resistance calls rely more on the dense discriminative signal in the mutation vector, while the KG branch contributes proportionally more for susceptible or ambiguous samples.
...
\end{equation}
The 0.3 auxiliary weight was selected by grid search on validation macro-F1, balancing primary task accuracy against symbolic regularization.


% ----------------------------------------------------------------
% Figure 2 -- 4-Stage Architecture
% ----------------------------------------------------------------
\begin{figure*}[t]
  \centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  >=Stealth,
  arr/.style={->, thick, black},
  arrw/.style={->, thick, white},
  elbl/.style={font=\scriptsize, align=center, fill=white, inner sep=1pt},
  %% Node styles
  bblue/.style={rectangle, rounded corners=4pt, draw=black,
    fill=cyan!65!blue, text=white, align=center,
    font=\small, inner sep=4pt,
    minimum width=3.8cm, minimum height=1.0cm},
  bpurple/.style={rectangle, rounded corners=4pt, draw=black,
    fill=violet!75, text=white, align=center,
    font=\small, inner sep=4pt,
    minimum width=3.8cm, minimum height=1.0cm},
  bgray/.style={rectangle, rounded corners=3pt, draw=black,
    fill=gray!22, text=black, align=center,
    font=\small, inner sep=3pt,
    minimum width=2.4cm, minimum height=0.65cm},
  borange/.style={rectangle, rounded corners=4pt, draw=black,
    fill=orange!85!red, text=white, align=center,
    font=\small, inner sep=4pt,
    minimum width=3.8cm, minimum height=1.1cm},
  bred/.style={rectangle, rounded corners=4pt, draw=black,
    fill=red!65, text=white, align=center,
    font=\small, inner sep=4pt,
    minimum width=3.8cm, minimum height=1.1cm},
  bgreen/.style={rectangle, rounded corners=4pt, draw=black,
    fill=green!55!black, text=white, align=center,
    font=\small, inner sep=4pt,
    minimum width=3.8cm, minimum height=1.0cm},
  bteal/.style={rectangle, rounded corners=4pt, draw=black,
    fill=teal!70, text=white, align=center,
    font=\small, inner sep=4pt,
    minimum width=3.8cm, minimum height=1.0cm},
  blabel/.style={font=\scriptsize\bfseries, text=white,
    fill=gray!50!black, inner sep=3pt,
    minimum width=1.2cm},
]

%% ===== STAGE HEADERS =====
\node[font=\scriptsize\bfseries, text=gray!60!black] at (-3.2, 1.35)
  {STAGE 1: INPUTS};
\node[font=\scriptsize\bfseries, text=gray!60!black] at (1.6, 1.35)
  {STAGE 2: ENCODING LANES};
\node[font=\scriptsize\bfseries, text=gray!60!black] at (6.5, 1.35)
  {STAGE 3: UNIFICATION};
\node[font=\scriptsize\bfseries, text=gray!60!black] at (11.5, 1.35)
  {STAGE 4: OUTPUTS \& GROUNDING};

%% ========== STAGE 1: INPUTS ==========
\node[bblue, text width=3.5cm, minimum height=1.3cm] (ginput) at (-3.2, 0.0)
  {\textbf{Genomic Input}\\[2pt]
   Mutation Vector\\
   $\mathbf{x}\!\in\!\{0,1\}^{17{,}352}$\\
   \scriptsize(37,761 isolates)};

\node[bpurple, text width=3.5cm, minimum height=1.3cm] (kinput) at (-3.2, -3.2)
  {\textbf{Symbolic Input}\\[2pt]
   MTB Knowledge Graph\\
   RotatE Embeddings\\
   \scriptsize(60,017 triples, 26 genes)};

%% ========== STAGE 2: ENCODING LANES ==========
\node[bblue, text width=3.6cm, minimum height=1.5cm] (genc) at (1.6, 0.0)
  {\textbf{Genomic Encoder}\\
   \textit{(Neural Component)}\\[2pt]
   2-Layer MLP\\
   BN $\to$ ReLU $\to$ Dropout\\
   $\mathbf{g}\!\in\!\mathbb{R}^{256}$};

\node[bpurple, text width=3.6cm, minimum height=1.5cm] (kenc) at (1.6, -3.2)
  {\textbf{KG Attention Encoder}\\
   \textit{(Symbolic Component)}\\[2pt]
   Scaled Dot-Product\\
   Attention over $\|\mathbf{v}\|$\\
   $\mathbf{k}\!\in\!\mathbb{R}^{64}$};

%% Dashed lane borders
\draw[rounded corners=5pt, gray!45, dashed, thick]
  (-0.2, 0.85) rectangle (3.6, -0.85);
\draw[rounded corners=5pt, gray!45, dashed, thick]
  (-0.2, -2.35) rectangle (3.6, -4.05);

%% Arrows Stage 1 -> Stage 2
\draw[arr] (ginput.east) -- (genc.west)
  node[midway, above, font=\scriptsize] {$\mathbf{x}$};
\draw[arr] (kinput.east) -- (kenc.west)
  node[midway, below, font=\scriptsize] {$\mathbf{E}\!\in\!\mathbb{R}^{26\times64}$};

%% ========== STAGE 3: UNIFICATION ==========
%% Projection nodes
\node[bgray] (gproj) at (5.4,  0.0) {$\tilde{\mathbf{g}}$ proj (128d)};
\node[bgray] (kproj) at (5.4, -3.2) {$\tilde{\mathbf{k}}$ proj (128d)};

\draw[arr] (genc.east) -- (gproj.west);
\draw[arr] (kenc.east) -- (kproj.west);

%% Gate
\node[borange, text width=3.8cm, minimum height=1.6cm] (gate) at (7.6, -1.6)
  {\textbf{Epistemic Trust Gate} $\boldsymbol{\alpha}$\\[3pt]
   $\boldsymbol{\alpha} = \sigma\!\left(\mathrm{MLP}([\tilde{\mathbf{g}};\tilde{\mathbf{k}}])\right)$\\[2pt]
   \scriptsize Resistant: $\bar{\alpha}\!=\!0.34$\\
   \scriptsize Susceptible: $\bar{\alpha}\!=\!0.22$};

\draw[arr] (gproj.east) to[out=0,in=150] (gate.west);
\draw[arr] (kproj.east) to[out=0,in=210] (gate.west);

%% Fused representation
\node[bred, text width=3.8cm, minimum height=1.3cm] (fused) at (7.6, -4.5)
  {\textbf{Fused Representation} $\mathbf{h}$\\[3pt]
   $\mathbf{h} = \boldsymbol{\alpha}\!\odot\!\tilde{\mathbf{g}}
   + (\mathbf{1}-\boldsymbol{\alpha})\!\odot\!\tilde{\mathbf{k}}$\\
   $\mathbf{h}\!\in\!\mathbb{R}^{128}$};

\draw[arr] (gate.south) -- (fused.north);

%% ========== STAGE 4: OUTPUTS & GROUNDING ==========
%% AMR Classifier
\node[bred, text width=3.8cm, minimum height=1.2cm] (amr) at (12.0, -1.6)
  {\textbf{AMR Prediction Head}\\
   $128\!\to\!64\!\to\!2$\\
   (Resistant / Susceptible)\\
   \textit{Primary Loss: CrossEntropy}};

%% Gene Auxiliary Head
\node[bteal, text width=3.8cm, minimum height=1.2cm] (aux) at (12.0, -4.0)
  {\textbf{Gene Auxiliary Head}\\
   (26-gene multi-label)\\
   \textit{Symbolic Regularizer}\\
   $\mathcal{L}_\text{aux}\!=\!0.3\,\mathcal{L}_\text{BCE}$};

\draw[arr] (fused.east) to[out=0,in=200] (amr.west);
\draw[arr] (fused.east) to[out=0,in=160] (aux.west);

%% Grounding outputs
\node[bgreen, text width=3.8cm, minimum height=1.3cm] (shap) at (12.0, -6.8)
  {\textbf{Level 1: SHAP Attribution}\\
   \textit{(Primary Trust Layer)}\\
   Mutation-level scores\\
   via GradientExplainer};

\node[bgreen, text width=3.8cm, minimum height=1.3cm] (kgtrace) at (12.0, -9.1)
  {\textbf{Level 2: KG Grounding}\\
   \textit{(Symbolic Verification)}\\
   Gene$\to$Mut$\to$Drug path\\
   BGR metric; Uncertainty flag};

\draw[arr] (amr.south) to[out=270,in=90] (shap.north);
\draw[arr] (shap.south) -- (kgtrace.north);

%% Outer border
\draw[rounded corners=7pt, black, very thick]
  (-5.3, 1.75) rectangle (14.1, -10.2);
\node[font=\normalsize\bfseries, anchor=north] at (4.4, 1.75)
  {KG-TRACE: Neuro-Symbolic Architecture};

\end{tikzpicture}}
  \caption{KG-TRACE four-stage neuro-symbolic architecture. Stage~1 and 2 process neural (genomic) and symbolic (KG) inputs independently. Stage~3 fuses them via an epistemic trust gate ($\boldsymbol{\alpha}$). Stage~4 outputs predictions, regularization signals, and dual-level mechanistic grounding reports.}
  \label{fig:arch}
\end{figure*}
...
KG-TRACE is optimized using Adam (learning rate $10^{-3}$, batch size~64) with early stopping on validation macro-F1 (patience~10, maximum 100~epochs; best checkpoint at epoch~3, wall time $\approx$4~min on an Apple M4). The full model has 9,142,493 parameters.
...
On the primary INH task, KG-TRACE reaches AUROC~0.9757 and macro-F1~0.9594. The fully-tuned baselines are competitive: LinearSVC reaches AUROC~0.9794 and Random Forest~0.9806, both with 22\% more training data. KG-TRACE matches these figures on fewer samples and delivers a mechanistic grounding that none of the baselines support.
...
\multirow{5}{*}{\parbox{1.8cm}{Baselines\\(tuned)}} &
  KG-TRACE (primary)$^\dagger$ & INH & \textbf{0.9757} & \textbf{0.9594} \\
...
\bottomrule
\multicolumn{5}{l}{\footnotesize $^\dagger$Converged model (primary); $^\ddagger$Same architecture, fixed ablation budget.}
\end{tabular}}
\end{table}
...
The targeted ablation on the auxiliary head (\textit{no\_aux\_head}) costs 3~AUROC points and 24~F1 points in the decimal. Small, but consistent: without the gene-detection objective, the gate can suppress the KG branch entirely, and the prediction is no longer symbolically grounded.
...
To verify that the neuro-symbolic framework generalizes beyond isoniazid, we evaluated KG-TRACE on three additional first-line and second-line drugs: Rifampicin (RIF), Ethambutol (EMB), and Levofloxacin (LEV). The model was trained from scratch for 10 epochs on each drug's respective sub-cohort. As shown in Table~\ref{tab:multidrug}, KG-TRACE maintains strong predictive performance across all three drugs. The AUROC for Rifampicin is exceptionally high (0.9853), reflecting the well-characterized nature of RIF resistance mechanisms (e.g., \textit{rpoB} mutations). Ethambutol and Levofloxacin show slightly lower F1-macro scores due to higher class imbalance and more complex, polygenic resistance patterns, but AUROC remains robustly above 0.94 in both cases.
...
Among all resistant predictions, 94.8\% trace to at least one gene$\to$mutation$\to$drug path in the MTB KG. For the remaining 5.2\%, the top-ranked SHAP mutation has no KG entry connecting it to the drug of interest, and the system issues an UNCERTAIN Actionability Score and a laboratory follow-up flag. The two anomalous SHAP features fall into this 5.2\%: the KG trace finds no direct path from \textit{tlyA}:L11L or \textit{mmpL5}:I948V to INH, confirming them as co-occurrence artefacts and keeping them out of the clinical note.

\subsubsection{Biological Grounding Ratio (BGR)}

To put a number on how well the neural attributions align with the symbolic knowledge base, we define the \textbf{BGR}: the fraction of top-$k$ SHAP features (across the test set) that have a corresponding KG node connected to the drug of interest. For the INH test set with $k{=}50$:
\begin{equation}
  \text{BGR}@50 = \frac{\left| \begin{array}{@{}c@{}} \text{top-50 SHAP features} \\ \text{with KG path to INH} \end{array} \right|}{50} = \frac{44}{50} = 0.88
\end{equation}

The remaining 12\% of high-SHAP features---either co-occurrence noise or uncatalogued variants---are precisely the cases flagged by the UNCERTAIN Actionability Score. A BGR of 0.88 also means the model's 'reasoning' is not merely statistical: 88\% of the model's most influential features are covered by the symbolic constraint, so the verification step is doing substantive work.
...
SHAP via GradientExplainer runs on CPU and introduces non-trivial latency at the cohort scale. Approximate SHAP variants or hardware acceleration would be needed before deployment in a real-time clinical pipeline.

\noindent\textbf{Clinical validation.}
Everything here is retrospective. The biological plausibility of the outputs---94.8\% of resistant predictions linked to WHO-catalogued pathways, BGR@50 of 0.88, top SHAP mutations consistent with established clinical knowledge---is indirect evidence of utility. A prospective study with pre-defined endpoints (time-to-correct-treatment-initiation, clinician concordance with the grounding report) and regulatory oversight would be required before any clinical deployment. A practical starting point would be a retrospective clinician review in which treating physicians and microbiologists rate KG-TRACE Decision Support Notes on actionability and biological coherence.

% -- 6. Conclusion --------------------------------------------
\section{Conclusion}\label{sec:conclusion}

We have described KG-TRACE, a neuro-symbolic framework for AMR prediction that connects neural genomic patterns to the structured biological knowledge in the WHO mutation catalogue. On the CRyPTIC \textit{M.~tuberculosis} cohort, the framework matches fully-tuned baselines trained on more data while providing a two-level grounding chain: SHAP attributions checked against WHO evidence grades (Level~1), verified by KG pathway tracing with 94.8\% symbolic coverage (Level~2), and summarized by the Biological Grounding Ratio at 0.88. The Actionability Score protocol - automatically flagging predictions whose top SHAP features lack KG paths to the drug of interest- gives clinicians a concrete signal about whether to trust a grounding report or request laboratory follow-up. Early results on rifampicin show similar KG pathway coverage (above 90\%), suggesting the approach is not specific to isoniazid; a full multi-drug analysis is planned for future work.
