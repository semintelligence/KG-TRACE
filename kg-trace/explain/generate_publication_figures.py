#!/usr/bin/env python3
"""
Generate all publication-quality figures and tables for KG-Trace paper.
...
def fig2_architecture_with_gate_inset():
    ...
    ax.set_ylim(0, 12)
    ax.axis("off")
    ax.set_title("KG-Trace Architecture — Hybrid Cross-Attention Fusion Model",
...
def fig2b_confusion_matrices():
    ...
    models = {
        "KG-Trace (Ours)": (to["preds"], to["labels"]),
...
# ====================================================================
def fig9_roc_curves():
    """ROC curves for KG-Trace and all baselines."""
    to = np.load("model/test_outputs.npz", allow_pickle=True)
    bp = np.load("baselines/baseline_predictions.npz", allow_pickle=True)

    curves = {
        "KG-Trace (Ours)": (to["probs"], to["labels"]),
        "SVM":               (bp["svm_probs"], bp["y_test"]),
        "XGBoost":           (bp["xgb_probs"], bp["y_test"]),
        "Random Forest":     (bp["rf_probs"], bp["y_test"]),
    }
    colors = {"KG-Trace (Ours)": C_FUSED, "SVM": C_DATA,
    ...
    fig.update_layout(
        title=dict(text="Figure 9: ROC Curves — INH Test Set (n = 5,665)<br>"
                        "<sub>KG-Trace achieves competitive AUROC while providing biological interpretability</sub>",
                   font=dict(size=14)),
...
def fig13_multidrug_confusion():
    ...
    fig.update_layout(
        title=dict(text="Figure 13: KG-Trace Multi-Drug Confusion Matrices (M. tuberculosis)<br>"
...
def fig14_species_roc():
    ...
    Experiment verified: same KGTrace architecture trained per-species
    ...
    fig.update_layout(
        title=dict(
            text="Figure 14: ROC Curves — Multi-Species Extension<br>"
                 "<sub>Same KG-Trace architecture per species. "
...
def fig15_species_comparison():
    ...
    fig.update_layout(
        title=dict(text="Figure 15: KG-Trace Performance Across Species<br>"
...
def fig_model_comparison_table():
    ...
    # Build table rows
    rows = []
    # KG-Trace full (100 epochs)
    full_ab = [a for a in ab if a["config"] == "full_kg_trace"][0]
    kgtrace = [m for m in fr if m["Model"] == "KG-Trace"][0]
    rows.append([
        "KG-Trace\n(Full, 100ep)", "9,142,493",
        "~130 s", "MPS/GPU",
        kgtrace["AUROC"], kgtrace["F1-macro"],
        kgtrace["Precision"], kgtrace["Recall"],
        "128", "64", "0.3",
        str(kgtrace.get("Gate_mean", "0.338")),
        "Cross-Attn"
    ])

    # Ablation variants (10 epochs)
    for a in ab:
        if a["config"] == "full_kg_trace":
            label = "KG-Trace\n(10ep)"
        else:
...
def fig_training_curve():
    ...
    fig.update_layout(
        title=dict(text="Training Curve — KG-Trace on INH (100 Epochs)<br>"
...
def fig3_multi_dataset_auroc():
    ...
    fig.update_layout(
        title=dict(text="Figure 3b: KG-Trace AUROC Across Drug Datasets<br>"
...
if __name__ == "__main__":
    print("=" * 70)
    print("GENERATING ALL PUBLICATION FIGURES & TABLES FOR KG-Trace")
    print("=" * 70)
