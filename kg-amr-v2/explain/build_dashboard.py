...
all_models_cm = [
    ("KG-AMR v2", kg_results["confusion_matrix"]),
]
for br in baseline_results:
...
<head>
<meta charset="utf-8">
<title>KG-AMR v2 — Explainability Dashboard</title>
<style>
    body {{
        background: #0a0a1a;
        color: #e0e0e0;
...
<body>
<h1>🧬 KG-AMR v2 — Explainability Dashboard</h1>
...
<div class="honesty">
    ...
    via sklearn/scipy. No values are hardcoded. KG-AMR v2 underperforms SVM on AUROC 
...
<div class="metric-grid">
    <div class="metric-card">
        ...
        <div class="label">KG-AMR v2 AUROC</div>
    </div>
    <div class="metric-card">
        <div class="value">{kg_results['f1_macro']:.4f}</div>
        <div class="label">KG-AMR v2 F1-macro</div>
    </div>
    <div class="metric-card">
...
<div class="honesty">
<ul>
    <li><strong>AUROC:</strong> SVM ({baseline_results[0]['auroc']:.4f}) > XGBoost ({baseline_results[1]['auroc']:.4f}) > KG-AMR v2 ({kg_results['auroc']:.4f}) > RF ({baseline_results[2]['auroc']:.4f}). KG-AMR v2 is <strong>not</strong> the best on raw AUROC.</li>
    <li><strong>F1-macro:</strong> XGBoost ({baseline_results[1]['f1_macro']:.4f}) ≈ KG-AMR v2 ({kg_results['f1_macro']:.4f}) ≈ SVM ({baseline_results[0]['f1_macro']:.4f}) >> RF ({baseline_results[2]['f1_macro']:.4f}).</li>
    <li><strong>BCS@10:</strong> KG-AMR v2 = {alignment.get('bcs_global_10', 'n/a')} — all top-10 SHAP features map to CARD genes (same for XGBoost/RF baselines since all features are named by gene).</li>
