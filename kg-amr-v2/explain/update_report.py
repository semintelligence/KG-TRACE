...
fig_auroc.update_layout(
    title="KG-AMR — Multi-Drug Performance (CRyPTIC MTB)",
    barmode="group",
...
colors = ["#ffd700" if c == "full_kg_amr" else "#00d2ff" for c in config_names]
...
fig_ablation.add_trace(go.Bar(
    ...
    marker_color=["#ffc107" if c == "full_kg_amr" else "#00ff88" for c in config_names],
...
<p style="color:#888">KG-AMR trained on {len(mtb_results) + 1} MTB drug datasets using shared mutation features and RotatE embeddings. All metrics from real model outputs.</p>
...
<p style="color:#888">Five model configurations trained for 10 epochs on the INH dataset using the same train/test split. Gold bars = full KG-AMR.</p>
...
full_auroc = next((r["auroc"] for r in ablation_results if r["config"] == "full_kg_amr"), 0)
for r in ablation_results:
    delta = r["auroc"] - full_auroc
    highlight = ' style="background:#2a2a0e;"' if r["config"] == "full_kg_amr" else ""
    new_sections += f"""
...
updated_html = updated_html.replace(
    ...
    f"Generated by KG-AMR Pipeline • {len(mtb_results)+1} drug datasets • {len(ablation_results)} ablation configs • All metrics real"
)
