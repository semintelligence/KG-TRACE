...
Run from KG-AMR/ directory with the miniconda Python.
...
class KGAMRNoAux(pl.LightningModule):
    """Full KG-AMR architecture but with gene_head loss weight = 0."""
...
model_noaux = KGAMRNoAux(kmer_dim=KMER_DIM, num_genes=NUM_GENES)
...
if best_ckpt:
    model_noaux = KGAMRNoAux.load_from_checkpoint(best_ckpt, kmer_dim=KMER_DIM, num_genes=NUM_GENES)

auroc_noaux, f1_noaux, cm_noaux = evaluate_model(model_noaux, test_loader)

exp1_result = {
    "experiment": "Exp1_NoAuxHead",
    "description": "Full KG-AMR, loss = loss_amr only (no 0.3*loss_gene)",
...
ABLATION_CONFIGS = [
    ...
    ("full_kg_amr", "Full KG-AMR (100 epochs, early stopping)", KGAMR),
]
...
all_results = {
    ...
    "experiment_3_note": "Documentation only: baselines trained on train+val (32,096 samples). KG-AMR uses train-only (26,432 samples). State this in paper.",
...
print(f"  Full KG-AMR WITH aux head:    AUROC=0.9757, F1=0.9594  (from test_results.json)")
print(f"  Full KG-AMR WITHOUT aux head: AUROC={exp1_result['auroc']:.4f}, F1={exp1_result['f1_macro']:.4f}")
